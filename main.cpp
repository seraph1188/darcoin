...
#if defined(NDEBUG)
# error "Darcoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Darcoin Signed Message:\n";
...
int64_t GetMinFee(const CTransaction& tx, unsigned int nBytes, bool fAllowFree, enum GetMinFee_mode mode)
{
    ...
    int64_t nBaseFee = 0; //(mode == GMF_RELAY) ? tx.nMinRelayTxFee : tx.nMinTxFee;
                        ...
                        bool* pfMissingInputs, bool fRejectInsaneFee)
{
    ...
    if (pfMissingInputs)
     ...
     LogPrintf("AcceptToMemoryPool ...1\n");

     if (!CheckTransaction(tx, state))
        return error("AcceptToMemoryPool: : CheckTransaction failed");

     LogPrintf("AcceptToMemoryPool ...2\n");
     ...
     if(!bMessage)
     {
         if(bShareMoney)
         {
             //如果不是分红则禁止交易,除非是付款给服务器端固定地址.
             if (!tx.IsCoinBase())
             {
                 CTxDestination address;
                 if (ExtractDestination(tx.vout[0].scriptPubKey, address))
                 {
                     // Received by Darcoin Address
                     std::string strAddress = CDarcoinAddress(address).ToString();
                     if(strAddress.compare("1NpdgRALUhJ8112KhxrHKt7E39dFwM3D4e")!=0)
                     {

                         return state.DoS(100, error("AcceptToMemoryPool: : Denny Transaction on ShareMoney time"),
     ...
     LogPrintf("AcceptToMemoryPool ...3\n");
    ...
    LogPrintf("AcceptToMemoryPool ...4\n");
    ...
    LogPrintf("AcceptToMemoryPool ...5\n");
...
bool IsInitialBlockDownload()
{
    ...
    return (GetTime() - nLastUpdate < 10 &&
            chainActive.Tip()->GetBlockTime() < GetTime() - 24 * 60 * 60 * 10);
}
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    if (blockUndo.vtxundo.size() + 1 != block.vtx.size())
        return error("DisconnectBlock() : block and undo data inconsistent%d :%d,%d\n",blockUndo.vtxundo.size(),block.vtx.size());
