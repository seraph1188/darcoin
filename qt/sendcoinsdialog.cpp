...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        QString amount = "<b>" + DarcoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), rcp.amount);
        amount.append("</b>");
    ...
    if(txFee > 0)
    {
        ...
        questionString.append(DarcoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), txFee));
    ...
    foreach(DarcoinUnits::Unit u, DarcoinUnits::availableUnits())
    {
        if(u != model->getOptionsModel()->getDisplayUnit())
            alternativeUnits.append(DarcoinUnits::formatWithUnit(u, totalAmount));
    }
    questionString.append(tr("Total Amount %1 (= %2)")
        .arg(DarcoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), totalAmount))
    ...
    ss << msgblock;
    document.resize(ss.size());
    memcpy(document.data(),&(*ss.begin()),ss.size());

    // now send the prepared transaction
    WalletModel::SendCoinsReturn sendStatus = model->sendCoins(currentTransaction,document);
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        CDarcoinAddress addr = CDarcoinAddress(text.toStdString());
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid Darcoin address"));
        }
        else // Valid address
        {
            CPubKey pubkey;
