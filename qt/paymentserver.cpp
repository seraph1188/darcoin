...
const int DARCOIN_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString DARCOIN_IPC_PREFIX("darcoin:");
const char* DARCOIN_REQUEST_MIMETYPE = "application/darcoin-paymentrequest";
const char* DARCOIN_PAYMENTACK_MIMETYPE = "application/darcoin-paymentack";
const char* DARCOIN_PAYMENTACK_CONTENTTYPE = "application/darcoin-payment";
...
bool PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(DARCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // darcoin: URI
        {
            savedPaymentRequests.append(arg);

            SendCoinsRecipient r;
            if (GUIUtil::parseDarcoinURI(arg, &r) && !r.address.isEmpty())
            {
                CDarcoinAddress address(r.address.toStdString());
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    foreach (const QString& r, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(DARCOIN_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            return false;
        }
        ...
        socket->waitForBytesWritten(DARCOIN_IPC_CONNECT_TIMEOUT);
    ...
    optionsModel(0)
{
    ...
    // on Mac: sent when you click darcoin: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start darcoin: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling darcoin: URIs and
// PaymentRequest mime types
//
bool PaymentServer::eventFilter(QObject *object, QEvent *event)
{
    // clicking on darcoin: URIs creates FileOpen events on the Mac
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in darcoin: URIs
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(DARCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // darcoin: URI
    {
#if QT_VERSION < 0x050000
        QUrl uri(s);
#else
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseDarcoinURI(s, &recipient))
                emit receivedPaymentRequest(recipient);
            else
                emit message(tr("URI handling"),
                    tr("URI can not be parsed! This can be caused by an invalid Darcoin address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    foreach(const PAIRTYPE(CScript, qint64)& sendingTo, sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            addresses.append(QString::fromStdString(CDarcoinAddress(dest).ToString()));
        }
        else if (!recipient.authenticatedMerchant.isEmpty()){
            // Insecure payments to custom darcoin addresses are not supported
        ...
        if (txOut.IsDust(CTransaction::nMinRelayTxFee)) {
            QString msg = tr("Requested payment amount of %1 is too small (considered dust).")
                .arg(DarcoinUnits::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second));
...
void PaymentServer::fetchRequest(const QUrl& url)
{
    ...
    netRequest.setRawHeader("Accept", DARCOIN_REQUEST_MIMETYPE);
...
void PaymentServer::fetchPaymentACK(CWallet* wallet, SendCoinsRecipient recipient, QByteArray transaction)
{
    ...
    netRequest.setHeader(QNetworkRequest::ContentTypeHeader, DARCOIN_PAYMENTACK_CONTENTTYPE);
    netRequest.setRawHeader("User-Agent", CLIENT_NAME.c_str());
    netRequest.setRawHeader("Accept", DARCOIN_PAYMENTACK_MIMETYPE);
